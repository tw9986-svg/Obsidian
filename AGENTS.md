...
- **`02_Wiki/`** — Codex가 소유·관리하는 서술/개념 계층. 사람은 읽기만 하고, 쓰기·상호 링크 유지는 Codex가 담당.
...
- **`04_Projects/`** — 연구 진행상황 문서. 시스템별 "현재 상태" 페이지 (`msre-transform-status.md` 등). 원본 연구노트는 `01_Raw/`에 보존하고, 이 페이지는 Codex가 계속 갱신하는 living document.
...
Codex Desktop에서 **Open folder**로 `D:\Reserch`를 직접 열어 작업한다. 이 폴더가 Obsidian Vault와 동일한 실제 경로이므로 Desktop에서 생성·수정한 파일은 별도 복사나 REST API 없이 즉시 Vault에 반영된다.
...
상세 절차는 `.Codex/skills/wiki-ingest/SKILL.md`에 스킬로 고정되어 있습니다. 요약:
